...
#include "wheat_seeds_dataset.h"       // Contém wheat_features[210][7] e wheat_labels[210]
#include "wheat_seeds_normalization.h" // Contém os vetores mean[7] e std_dev[7]

#define NUM_SAMPLES 210            // Total de amostras no dataset Wheat Seeds
#define NUM_CLASSES 3              // Wheat Seeds possui 3 classes (0, 1 e 2)
 ...
 */
void normalize_input(const float in[7], float out[7]) {
    for (int i = 0; i < 7; i++)
        out[i] = (in[i] - mean[i]) / std_dev[i];
}
...
int main() {
    ...
    printf("\n=== TinyML Wheat Seeds - Matriz de Confusao ===\n");
    ...
    printf("Iniciando inferencia nas 210 amostras do dataset Wheat Seeds...\n");
     ...
     * Percorre todas as 210 amostras do dataset Wheat Seeds.
     */
    for (int i = 0; i < NUM_SAMPLES; i++) {
        float features_norm[7];    // Entrada normalizada (7 atributos)
        float scores[3];           // Saída do modelo (probabilidades para 3 classes)
        
        // Normaliza a entrada usando média e desvio salvos
        normalize_input(wheat_features[i], features_norm);
